struct Hash {
    ...
    void preHash() {
        ...
        for(int i = 1 ; i < n ; i++) {
            ...
            ha1[i] = Add(Mul(ha1[i-1],p1,m1),(s[i]-dif),m1);
            ha2[i] = Add(Mul(ha2[i-1],p2,m2),(s[i]-dif),m2);
        }
    }
    void build_hash() {
        if(!ha1.empty()) {
        ...
        hash.first = hash.second = s[0]-dif;
        for(int i = 1 ; i < n ; i++) {
            hash.first = Add(Mul(hash.first,p1,m1),(s[i]-dif),m1);
            hash.second = Add(Mul(hash.second,p2,m2),(s[i]-dif),m2);
        }
    }
